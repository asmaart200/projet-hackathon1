...
st.sidebar.header("📋 État des Dépendances")
if not PLOTLY_AVAILABLE:
    st.sidebar.error("⚠️ Plotly non disponible")
    st.sidebar.code("pip install plotly")
else:
    st.sidebar.success("✅ Plotly disponible")

if not SKLEARN_AVAILABLE:
    st.sidebar.error("⚠️ Scikit-learn non disponible")
    st.sidebar.code("pip install scikit-learn")
else:
    st.sidebar.success("✅ Scikit-learn disponible")

if not SCIPY_AVAILABLE:
    st.sidebar.error("⚠️ Scipy non disponible")
    st.sidebar.code("pip install scipy")
else:
    st.sidebar.success("✅ Scipy disponible")
    ...
    for country in df['Pays'].unique():
        ...
        if len(country_data) > 0:
            stats_dict[country] = {
                ...
                'Tendance': 'Croissante' if len(country_data) > 1 and country_data.iloc[-1] > country_data.iloc[0] else 'Décroissante',
                'Variation (%)': ((country_data.iloc[-1] - country_data.iloc[0]) / country_data.iloc[0] * 100) if len(country_data) > 1 and country_data.iloc[0] != 0 else 0
            }
...
def perform_regression_analysis(df, degree=1):
    ...
    if not SKLEARN_AVAILABLE:
        st.error("Scikit-learn requis pour la modélisation")
        return {}
    ...
    for country in df['Pays'].unique():
        ...
        try:
            ...
            st.warning(f"Erreur dans la modélisation pour {country}: {str(e)}")
            continue
...
def create_correlation_matrix(df):
    ...
    if not PLOTLY_AVAILABLE:
        st.error("Plotly requis pour les graphiques")
        return None
    ...
    try:
        ...
        st.error(f"Erreur dans la création de la matrice de corrélation: {str(e)}")
        return None

def create_basic_chart(df, chart_type="line"):
    """Crée un graphique de base sans Plotly"""
    if chart_type == "line":
        st.line_chart(df.pivot(index='Année', columns='Pays', values='Valeur'))
    elif chart_type == "bar":
        latest_data = df.groupby('Pays')['Valeur'].last()
...
# Sélection des pays
default_countries = []
for country in ['Maroc', 'France', 'Allemagne', 'États-Unis']:
    if country in country_names:
        default_countries.append(country)
    if len(default_countries) >= 3:
        break

if not default_countries:
    default_countries = country_names[:3]
...
with col1:
    start_year = st.number_input("Année de début", min_value=1960, max_value=2023, value=2010)
with col2:
    end_year = st.number_input("Année de fin", min_value=1960, max_value=2023, value=2022)

# Validation des années
if start_year >= end_year:
    st.sidebar.error("L'année de début doit être inférieure à l'année de fin")
...
if 'df' in st.session_state and not st.session_state['df'].empty:
    ...
    with tab1:
        ...
        if PLOTLY_AVAILABLE:
            ...
            st.warning("Plotly n'est pas disponible. Utilisation des graphiques Streamlit de base.")
            
            # Graphiques de base
            st.subheader("Évolution temporelle")
            create_basic_chart(df, "line")
            
            st.subheader("Dernières valeurs")
            create_basic_chart(df, "bar")
            ...
            for country in df['Pays'].unique():
                ...
                if len(country_data) >= 3:
                    try:
                        ...
                        st.write(f"**{country}** : Analyse impossible")
        else:
            st.warning("Scipy requis pour l'analyse statistique avancée")
            # Analyse simple sans scipy
            for country in df['Pays'].unique():
                country_data = df[df['Pays'] == country].sort_values('Année')
                if len(country_data) >= 2:
                    first_val = country_data.iloc[0]['Valeur']
                    last_val = country_data.iloc[-1]['Valeur']
                    trend = "Croissante" if last_val > first_val else "Décroissante"
                    st.write(f"**{country}** : Tendance {trend}")
    ...
    with tab3:
        ...
        if SKLEARN_AVAILABLE:
            ...
            if regression_results:
                ...
                st.warning("Aucun modèle n'a pu être créé. Vérifiez les données.")
        else:
            st.error("Scikit-learn est requis pour la modélisation. Installez-le avec: pip install scikit-learn")
    ...
    st.markdown("""
    Pour utiliser toutes les fonctionnalités de cette application, installez les packages suivants :
